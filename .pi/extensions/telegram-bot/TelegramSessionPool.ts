...
import { getConfigDir, loadConfig } from "../../../src/utils/config.js";
...
export class TelegramSessionPool {
  ...
  handleMessage(userId: number, message: string): Promise<string> {
    const entry = this.getOrCreate(userId);
    const run = async (): Promise<string> => {
      const session = await entry.sessionPromise;
      return this.runPrompt(session, message);
    };
    entry.queue = entry.queue.then(run, run);
    return entry.queue;
  }

  private async runPrompt(session: AgentSession, message: string): Promise<string> {
    let settled = false;
