...
В Telegram: `/start`, `/status`, `/new`, `/setup` (в DM — все pending-группы,
в группе — настройка этой группы). Каждый пользователь получает **изолированную
сессию Гриши** (свой `sessionId`). Онбординг группы приходит прямо в группу
(с кнопками пресетов), пока группа не настроена бот молчит по контенту. Голосовые
