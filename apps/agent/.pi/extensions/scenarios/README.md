...
## Preset ≠ scenario (два «секретаря»)

| Кнопка в онбординге | Что это | Поведение |
|---------------------|---------|-----------|
| «Секретарь — тихий архив» | **scenario** `secretary` | молчит в группе, сохраняет историю; отчёты в личке (preset `listener`) |
| «Ассистент (отвечает по @)» | **preset** `secretary` | отвечает только по `@`/reply (`require_mention`) |

Кнопка сценария идёт ВЫШЕ кнопки пресета; слово «Секретарь» — только у сценария.
