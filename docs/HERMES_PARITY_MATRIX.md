...
| E4 | Injection/скрытые символы scan при записи | docs memory security | **W2**: scan подключён в `SqliteRagMemoryService.addFact` за флагом (block при zero-width/injection) | **PARTIAL** (wiring готов) | включение флага в prod | средний | E1 |
...
| E6 | Memory write_approval gate | docs memory | **W2**: gate подключён в `addFact` за флагом (goal/constraint или низкая confidence → требует approval, throw) | **PARTIAL** (wiring готов) | включение флага в prod | низкий | E1 |
...
| H4 | Depth limit, timeout, budget, toolset per worker | docs delegation | **Item 8.1**: `DelegationGuard` (maxDepth recursion protection, timeout, budget, maxWorkers, remaining*) в `src/runtime/delegation/limits.ts`; **W6**: гейт подключён в `createRealSubAgentRunner` за флагом (budget pre-check + timeout post-check) | **COMPLETE** (toolset per worker = toolset-конфиг subagent-сессий, существующее поведение) | — | — | — |
...
| P2 | Cost tracking по моделям | dashboard usage | **Item 16.2**: `TelemetryBuffer` (§31-события) + correlation ID (генерация/валидация) в `src/runtime/observability/telemetry.ts`; **W13**: `runtimeObservability` (события agent-run/model-selected/tokens/latency/error на каждый model-call за флагом) | **COMPLETE** (дашборд — отдельный шаг) | — | — | — |
| P3 | Session model usage | `session_model_usage` | **Item 16.1**: `SessionModelUsage` + `ModelUsageAccumulator` (totalsByRole для Model Router) + `estimateCost` (§32) в `src/runtime/observability/usage.ts`; **W13**: аккумуляция в `ModelRouter.call` за флагом (роль→usage→cost для Model Router) | **COMPLETE** | — | — | — |
...
MISSING: только F7 (slash-команды по скиллам, низкий приоритет). Оставшийся риск — в PARTIAL: B3 fallback (production), B5 отдельные aux-модели, C2 compaction
(production), F3 skill rollback (данные), I1 execute_code (безопасность),
