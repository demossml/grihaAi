...
| E6 | Memory write_approval gate | docs memory | **Item 11.4**: `memoryWriteNeedsApproval` (типы goal/constraint или низкая confidence → одобрение; конфигурируемая политика) в `src/runtime/security/approval.ts` | **PARTIAL** (gate готов) | wiring в memory-tools | низкий | E1 |
