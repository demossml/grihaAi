...
|---|---|---|
| COMPLETE | 35 | существующие Griha-возможности + все 13 wiring-подключений (W1–W13) + B3 fallback |
| PARTIAL | 32 | контракты/логика готовы; остаётся финальное включение рисковых подсистем (C2/F3/I1) и отдельные шаги (дашборд, runsc-MCP, thread_id, profile-override) |
...
B3 fallback — подключён (post-wiring, за флагом).
