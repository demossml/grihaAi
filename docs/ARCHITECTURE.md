...
- **Рендер**: `src/utils/reports/report-renderer.ts`:
  ...
  - **Кириллица**: `@json-render/react-pdf` жёстко шрифтует текст семействами Helvetica (без глифов кириллицы). Перед рендером `ensureReportFonts` подменяет их на TTF с кириллицей (Arial / DejaVu Sans), иначе текст рендерится как glyph 0 — визуально пустой лист. Шрифт ищется по системным путям (macOS/Linux/Windows); переопределение — `GRIHA_PDF_FONT_PATH` (+ опционально `GRIHA_PDF_FONT_BOLD_PATH`). Шрифт не найден → явная ошибка, а не «успешный» пустой PDF.
