...
import { discoverSkills, formatSkillsForPrompt } from "./index.js";

let root: string;

before(async () => {
  root = await fs.mkdtemp(path.join(os.tmpdir(), "griha-skills-"));
...
describe("discoverSkills", () => {
  ...
  it("supports the autoCreated flag and block tags", async () => {
    const skills = await discoverSkills(root);
  ...
  it("discovers the shipped core skill from the package root by default", async () => {
